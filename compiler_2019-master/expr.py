...
class Read(Expr):
    ...
    def gen(self, context: Context, target: str):
        """We read by loading memory mapped at address 510"""
...
class If(Control):
    ...
    def gen(self, context: Context, target: str):
        """Looping"""
        cond_else = context.new_label("else")      #Adding Labels Else and fi
        cond_exit = context.new_label("fi")

        self.cond.condjump(context, target, cond_else, jump_cond=False) #If Condition to Jump

        self.thenpart.gen(context, target)               #If condition satisfies execute then part
        context.add_line(f"   JUMP  {cond_exit}")       #Jump to exit so we don't execute else part

        context.add_line(f"{cond_else}:")         #Else part execution
        self.elsepart.gen(context, target)
        context.add_line(f"{cond_exit}:")       #Exit
